<?php
session_start();

if (!isset($_SESSION['user_id'])) {
    // El usuario no está autenticado, redirige a la página de inicio de sesión
    header("Location: index.php");
    exit();
}
?>
<!DOCTYPE html>
<html>
<head>
    <title>Ingresar Orden</title>
    <link rel="stylesheet" href="orden_form.css">
    <link href="https://fonts.googleapis.com/css2?family=Alfa+Slab+One&display=swap" rel="stylesheet">
    <link rel="stylesheet" href="https://use.fontawesome.com/releases/v5.6.3/css/all.css" integrity="sha384-UHRtZLI+pbxtHCWp1t77Bi1L4ZtiqrqD80Kn4Z8NTSRyMA2Fd33n5dQ8lWUE00s/" crossorigin="anonymous">
    <script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>
    <script>
        function agregarCampo() {
            // Crear un nuevo elemento de div para los campos
            var nuevoCampo = document.createElement("div");

            // Agregar el contenido HTML para los campos
            nuevoCampo.innerHTML = `
                <label for="">Producto ID:</label>
                <input type="number" name="producto_id[]" class="card-input producto-id">
                <input type="text" name="nombre_producto[]" readonly class="card-input nombre-producto" placeholder="Nombre Producto">
                <input type="number" name="cantidad[]" class="card-input cantidad" placeholder="Cantidad">
                <br>
            `;

            // Obtener el botón de agregar campo
            var botonAgregar = document.getElementById("botonAgregar");

            // Insertar el nuevo campo antes del botón de agregar
            botonAgregar.parentNode.insertBefore(nuevoCampo, botonAgregar);
        }

        $(document).ready(function () {
            var currentDate = new Date().toISOString().slice(0, 10);
            $("#fecha").val(currentDate);

            // Usar eventos delegados para manejar los campos dinámicos
            $(document).on("input", ".producto-id", function () {
                var productoId = $(this).val();
                var nombreProductoInput = $(this).closest("div").find(".nombre-producto");

                $.ajax({
                    url: "obtener_nombre_producto.php",
                    type: "GET",
                    data: { producto_id: productoId },
                    success: function (data) {
                        nombreProductoInput.val(data);
                    },
                });
            });

            $("#codigo_cliente").on("input", function () {
                var clienteCodigo = $(this).val();

                $.ajax({
                    url: "obtener_nombre_cliente.php",
                    type: "GET",
                    data: { cliente_codigo: clienteCodigo },
                    success: function (data) {
                        $("#nombre_cliente").val(data);
                    },
                });
            });
        });
    </script>

    <style>
      .buttoncerrar {
    -moz-appearance: none;
    -webkit-appearance: none;
    appearance: none;
    border: none;
    background: none;
    color: #0f1923;
    cursor: pointer;
    position: relative;

   
    text-transform: uppercase;
    font-weight: bold;
    font-size: 14px;
    transition: all .15s ease;
  }
        
.button {
    -moz-appearance: none;
    -webkit-appearance: none;
    appearance: none;
    border: none;
    background: none;
    color: #0f1923;
    cursor: pointer;
    position: relative;
    padding: 8px;
    margin-bottom: 20px;
    text-transform: uppercase;
    font-weight: bold;
    font-size: 14px;
    transition: all .15s ease;
  }
  
  .button::before,
  .button::after {
    content: '';
    display: block;
    position: absolute;
    right: 0;
    left: 0;
    height: calc(50% - 5px);
    border: 1px solid #7D8082;
    transition: all .15s ease;
  }
  
  .button::before {
    top: 0;
    border-bottom-width: 0;
  }
  
  .button::after {
    bottom: 0;
    border-top-width: 0;
  }
  
  .button:active,
  .button:focus {
    outline: none;
  }
  
  .button:active::before,
  .button:active::after {
    right: 3px;
    left: 3px;
  }
  
  .button:active::before {
    top: 3px;
  }
  
  .button:active::after {
    bottom: 3px;
  }
  
  .button_lg {
    position: relative;
    display: block;
    padding: 10px 20px;
    color: #fff;
    background-color: #0f1923;
    overflow: hidden;
    box-shadow: inset 0px 0px 0px 1px transparent;
  }
  
  .button_lg::before {
    content: '';
    display: block;
    position: absolute;
    top: 0;
    left: 0;
    width: 2px;
    height: 2px;
    background-color: #0f1923;
  }
  
  .button_lg::after {
    content: '';
    display: block;
    position: absolute;
    right: 0;
    bottom: 0;
    width: 4px;
    height: 4px;
    background-color: #0f1923;
    transition: all .2s ease;
  }
  
  .button_sl {
    display: block;
    position: absolute;
    top: 0;
    bottom: -1px;
    left: -8px;
    width: 0;
    background-color: #ff4655;
    transform: skew(-15deg);
    transition: all .2s ease;
  }
  
  .button_text {
    position: relative;
  }
  
  .button:hover {
    color: #0f1923;
  }
  
  .button:hover .button_sl {
    width: calc(100% + 15px);
  }
  
  .button:hover .button_lg::after {
    background-color: #fff;
  }
  
    </style>
</head>
<body>
<header>
  <!--  <div class="logo">  
        <img src="data:image/jpeg;base64," alt="Logo de tu empresa">
    </div> -->
     <nav>
        <ul>
        <li><a href="inde.php"><i class="fa fa-home"></i> Inicio</a></li>
            <li><a href="registro.php"><i class="fas fa-sharp fa-solid fa-users"></i> Clientes</a></li>
            <li><a href="orden_form.php"><i class='fas fa-clipboard-list'></i> Ordenes</a></li>
            <li><a href="formulario_orden.php"><i class="fas  fa-thin fa-cart-arrow-down"></i> Compras</a></li>
            <li><a href="ingresar_producto.php"><i class="fas fa-solid fa-tag"></i> Articulos</a></li>
            <li><a href="informe.php"><i class='fas fa-exclamation-circle'></i> Productos Agotados</a></li>
            <li><a href="cerrarSesion.php"><button class="buttoncerrar" type="button"><i class='fas fa-solid fa-lock'></i> </a></button></li>
        </ul>
    </nav>
 

</header>

<form action="procesar_orden.php" method="post">
    <div class="cuadro">
        <div class="cuadro2">
            <center>
                <p>Ordenes</p>
            </center>
            <label for="">ID:</label>
            <input type="number" name="id" class="card-input" id="id">
            <label for="fecha" style="margin-left: 550px;">Fecha orden:</label>
            <input type="date" id="fecha" name="fecha" class="card-input">
            <br>
            <label for="">Codigo cliente:</label>
            <input type="number" name="codigo" id="codigo_cliente" class="card-input">
            <label for="fecha" style="margin-left: 550px;">Fecha fin orden:</label>
            <input type="date" id="fechafin" name="fechafin" class="card-input">
            <br>
            <label for="">Nombre Cliente:</label>
            <input type="text" id="nombre_cliente" name="nombre_cliente" readonly class="card-input">
            <br><br><br>
            <hr class="blue-line">
        </div>
        <center>
            <div>
                <label for="">Producto ID:</label>
                <input type="number" name="producto_id[]" class="card-input producto-id">
                <input type="text" name="nombre_producto[]" class="card-input nombre-producto" readonly placeholder="Nombre Producto">
                <input type="number" name="cantidad[]" class="card-input cantidad" placeholder="Cantidad">
                <br>
            </div>

            <!-- Botón para agregar campos -->
            <button type="button" id="botonAgregar" onclick="agregarCampo()">Agregar Campo</button>
        </center>
        <label for="">Observación</label>
        <input type="text" id="observacion" name="observacion" class="card-input"><br>
    
        <label for="correciones">Correciones</label>
    <select type="text" id="correciones" name="correciones" >
        <option value="">Seleccione</option>
        <option value="SI">SI</option>
        <option value="NO">NO</option>
    </select>





<center>
        <button class="button" type="submit" value="Ingresar Orden" class="button type1">
            <span class="button_lg">
                <span class="button_sl"></span>
                <span class="button_text">Registrar</span>
            </span>
        </button></center>

    </div>
</form>





</body>
</html>
