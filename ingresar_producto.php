

<!DOCTYPE html>
<html>
<head>
    <title>Gestión de Productos</title>
    <link rel="stylesheet" href="ingresar_producto.css">
    <link href="https://fonts.googleapis.com/css2?family=Alfa+Slab+One&display=swap" rel="stylesheet">
    <link rel="stylesheet" href="https://use.fontawesome.com/releases/v5.6.3/css/all.css" integrity="sha384-UHRtZLI+pbxtHCWp1t77Bi1L4ZtiqrqD80Kn4Z8NTSRyMA2Fd33n5dQ8lWUE00s/" crossorigin="anonymous">
  
   <style>
.buttoncerrar {
  -moz-appearance: none;
  -webkit-appearance: none;
  appearance: none;
  border: none;
  background: none;
  color: #0f1923;
  cursor: pointer;
  position: relative;

 
  text-transform: uppercase;
  font-weight: bold;
  font-size: 14px;
  transition: all .15s ease;
}

    header {
  display: flex;
  justify-content:space-around;
  align-items: center;
}

nav {
  text-align: left; 
  font-family: 'Alfa Slab One', cursive;
  font-size: 15px;
 
}


nav ul {
  list-style: none;
  margin: 0;
  padding: 0;

}

nav li {
  display: inline;
  margin-right: 20px; /* Espacio entre los enlaces */
}

nav a {
  text-decoration: none;
  color: #000000;
  padding: 5px;
}
nav a:hover{
  background-color: #abc6d8;
  color: #FFFFFF;

border-radius: 15px;
}



.logo img{width: 100px; height: 50px;}</style>
</head>
<body>

<header>
  <!--  <div class="logo">  
        <img src="data:image/jpeg;base64," alt="Logo de tu empresa">
    </div> -->
     <nav>
        <ul>
       
            <li><a href="ingresar_producto.php"><i class="fas fa-solid fa-tag"></i> Reporte</a></li>
            <li><a href="informe.php"><i class='fas fa-exclamation-circle'></i>Informe de reporte</a></li>
            <li><a href="cerrarSesion.php"><button class="buttoncerrar" type="button"><i class='fas fa-solid fa-lock'></i> </a></button></li>
        </ul>
    </nav>
 

</header>
  

    <div class="cuadro2">
    <center>
        <p>PEDIDO SIN ENTREGA</p>
    </center>
    <br>
    <br>

    <!-- Formulario para ingresar o actualizar productos -->

    <form action="procesar_producto.php" method="POST" enctype="multipart/form-data">
  <center>  
  
        
        <label for="id">CODIGO CLIENTE:</label>
        <input type="text" name="id" id="id" class="card-input" required>
        
        <label for="nombre_producto">NOMBRE COMPLETO:</label>
        <input type="text" name="nombre_producto" id="nombre_producto" required class="card-input">
        <br>
        <label for="vendedor">VENDEDOR:</label>
        <input type="text" name="vendedor" id="vendedor" class="card-input">
        
        <label for="kam">KAM:</label>
        <input type="text" name="kam" id="kam" class="card-input">
        <br>
        <label for="ciudad">CIUDAD:</label>
        <input type="text" name="ciudad" id="ciudad" class="card-input">
        
        <label for="coach">COACH:</label>
        <input type="text" name="coach" id="coach" class="card-input">
            <br>
        <label for="fecha">FECHA NOVEDAD:</label>
        <input type="date" name="fecha" id="fecha" class="card-input">

       

     <br>
    </center>
    <br>
    <br>
</div>
    <center> 
        <hr class="blue-line">
       


        <button class="button" type="submit" name="accion" value="Agregar" class="button type1">
            <span class="button_lg">
                <span class="button_sl"></span>
                <span class="button_text">Enviar</span>
            </span>
        </button>
    </center> 




    </form>

    <script>
        // Obtener referencia al campo de entrada del ID y al campo del nombre del producto
        const idInput = document.getElementById("id");
        const nombreProductoInput = document.getElementById("nombre_producto");

        // Escuchar el evento de cambio en el campo del ID
        idInput.addEventListener("change", () => {
            const selectedId = idInput.value;
            if (selectedId !== "") {
                // Realizar una solicitud para obtener el nombre del producto por su ID
                fetch(`nombre_producto.php?id=${selectedId}`)
                    .then(response => response.text())
                    .then(nombre => {
                        if (nombre === 'Producto no encontrado') {
                            nombreProductoInput.value = "";
                            nombreProductoInput.removeAttribute("readonly");
                        } else {
                            nombreProductoInput.value = nombre;
                            nombreProductoInput.setAttribute("readonly", "readonly");
                        }
                    })
                    .catch(error => {
                        console.error("Error al obtener el nombre del producto:", error);
                    });
            } else {
                nombreProductoInput.value = "";
                nombreProductoInput.removeAttribute("readonly");
            }
        });

       // Obtener la fecha actual
    var currentDate = new Date();

// Formatear la fecha como "YYYY-MM-DD"
var formattedDate = currentDate.toISOString().slice(0, 10);

// Asignar la fecha al campo de formulario
document.getElementById("fecha").value = formattedDate;


   // Obtener referencia al campo de entrada del ID y al campo del nombre del producto
   const idvendedorInput = document.getElementById("id");
        const vendedorInput = document.getElementById("vendedor");

        // Escuchar el evento de cambio en el campo del ID
        idvendedorInput.addEventListener("change", () => {
            const selectedId = idvendedorInput.value;
            if (selectedId !== "") {
                // Realizar una solicitud para obtener el nombre del producto por su ID
                fetch(`nombre_vendedor.php?id=${selectedId}`)
                    .then(response => response.text())
                    .then(nombre => {
                        if (nombre === 'Producto no encontrado') {
                            vendedorInput.value = "";
                            vendedorInput.removeAttribute("readonly");
                        } else {
                            vendedorInput.value = nombre;
                            vendedorInput.setAttribute("readonly", "readonly");
                        }
                    })
                    .catch(error => {
                        console.error("Error al obtener el nombre del producto:", error);
                    });
            } else {
                vendedorInput.value = "";
                vendedorInput.removeAttribute("readonly");
            }
        });


         // Obtener referencia al campo de entrada del ID y al campo del nombre del producto
   const idkamInput = document.getElementById("id");
        const kamInput = document.getElementById("kam");

        // Escuchar el evento de cambio en el campo del ID
        idkamInput.addEventListener("change", () => {
            const selectedId = idkamInput.value;
            if (selectedId !== "") {
                // Realizar una solicitud para obtener el nombre del producto por su ID
                fetch(`nombre_kam.php?id=${selectedId}`)
                    .then(response => response.text())
                    .then(nombre => {
                        if (nombre === 'kam no encontrado') {
                            kamInput.value = "";
                            kamInput.removeAttribute("readonly");
                        } else {
                            kamInput.value = nombre;
                            kamInput.setAttribute("readonly", "readonly");
                        }
                    })
                    .catch(error => {
                        console.error("Error al obtener el nombre del kam:", error);
                    });
            } else {
                kamInput.value = "";
                kamInput.removeAttribute("readonly");
            }
        });
       // Obtener referencia al campo de entrada del ID y al campo del nombre del producto
       const idciudadInput = document.getElementById("id");
        const ciudadInput = document.getElementById("ciudad");

        // Escuchar el evento de cambio en el campo del ID
        idciudadInput.addEventListener("change", () => {
            const selectedId = idciudadInput.value;
            if (selectedId !== "") {
                // Realizar una solicitud para obtener el nombre del producto por su ID
                fetch(`nombre_ciudad.php?id=${selectedId}`)
                    .then(response => response.text())
                    .then(nombre => {
                        if (nombre === 'ciudad no encontrado') {
                            ciudadInput.value = "";
                            ciudadInput.removeAttribute("readonly");
                        } else {
                            ciudadInput.value = nombre;
                            ciudadInput.setAttribute("readonly", "readonly");
                        }
                    })
                    .catch(error => {
                        console.error("Error al obtener el nombre del ciudad:", error);
                    });
            } else {
                ciudadInput.value = "";
                ciudadInput.removeAttribute("readonly");
            }
        });
  // Obtener referencia al campo de entrada del ID y al campo del nombre del producto
  const idcoachInput = document.getElementById("id");
        const coachInput = document.getElementById("coach");

        // Escuchar el evento de cambio en el campo del ID
        idcoachInput.addEventListener("change", () => {
            const selectedId = idcoachInput.value;
            if (selectedId !== "") {
                // Realizar una solicitud para obtener el nombre del producto por su ID
                fetch(`nombre_coach.php?id=${selectedId}`)
                    .then(response => response.text())
                    .then(nombre => {
                        if (nombre === 'coach no encontrado') {
                            coachInput.value = "";
                            coachInput.removeAttribute("readonly");
                        } else {
                            coachInput.value = nombre;
                            coachInput.setAttribute("readonly", "readonly");
                        }
                    })
                    .catch(error => {
                        console.error("Error al obtener el nombre del ciudad:", error);
                    });
            } else {
                coachInput.value = "";
                coachInput.removeAttribute("readonly");
            }
        });


    </script>
   
</body>
</html>
